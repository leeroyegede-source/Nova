...
*Generated by Nova AI.*`;
  ...
  const downloadSavedProject = async (project: any) => {
    let filesToZip: Record<string, string> = {};
    try {
      filesToZip = JSON.parse(project.code);
    } catch {
      filesToZip = { "/App.js": project.code };
    }
    ...
    try {
      ...
      if (!filesToZip['/package.json'] && !filesToZip['package.json']) {
        zip.file('package.json', JSON.stringify({
          name: project.name.toLowerCase().replace(/[^a-z0-9]+/g, '-') || "nova-saved-app",
          version: "1.0.0",
          private: true,
          dependencies: {
            "react": "^18.2.0",
      ...
      const readmeContent = `# \${project.name} - Nova AI Generated Project
                    ...
                    <div key={p.id} className="flex flex-col bg-white/5 border border-white/10 p-4 rounded-xl hover:border-blue-500/30 transition-colors group">
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => downloadSavedProject(p)} className="p-1.5 text-gray-400 hover:text-white hover:bg-white/10 transition-colors rounded-lg" title="Download ZIP"><Download className="w-4 h-4" /></button>
                          <button onClick={() => deleteProject(p.id)} className="p-1.5 text-red-400/50 hover:text-red-400 hover:bg-red-400/10 transition-colors rounded-lg" title="Delete"><Trash2 className="w-4 h-4" /></button>
                        </div>
                      </div>
